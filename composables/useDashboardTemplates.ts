...
export function useDashboardTemplates() {
  ...
  const saveAsTemplate = (templateId: string, name: string, description?: string) => {
    const layout = isCustomMode.value ? customLayout.value : dashboardTemplates[currentTemplate.value].layout
